!> Template to create the main grid and the lambda grids for a user-defined RT model. Modify the appropriate points to make a compilable program. 
PROGRAM create_adap_grid_model
  use iso_fortran_env
 ...
 ! IF not create it !!!
  ...
  case (.FALSE.)
     ...
     call set_model_input    XXX modify this sub XXX 
...
!> this subroutine obtains the average dust extinction coefficient and the average stellar luminosity density of the current cell from the user-defined routines.
  subroutine calc_dens(x,y,z,cellsize,av_rho_dust,av_rho_stars)
  
    real(Kind=real64) :: x,y,z,cellsize,av_rho_dust,av_rho_stars
     
  av_rho_dust=  XXX insert function giving dust extinction coefficient at wavelength lambda (units pc^-1) XXX   
  av_rho_stars = XXX insert function giving stellar luminosity density at wavelength lambda (units W/Hz or erg/s/Hz )  XXX 
...
XXX add more subdivision_criteria if needed XXX 
...
END PROGRAM create_adap_grid_model
