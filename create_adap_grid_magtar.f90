...
!!$      av_rho_dust =av_rho_dust_ring(x,y,z,cellsize)
   
  if (dust_geometry == 'shell') then 
     av_rho_dust=av_rho_dust_shell(x,y,z,cellsize)
  else if (dust_geometry == 'cavity') then
     av_rho_dust=av_rho_dust_cavity(x,y,z,cellsize)
  else if (dust_geometry == 'wind') then
     av_rho_dust=av_rho_dust_wind(x,y,z,cellsize)
  else 
...
real(kind=real64) :: x,y,z,cellsize,tau,lum_stars,rad
integer :: ib=1 
  ...
  case('shell')
    subdivision= (((nlevel < max_lvl).and.((tau > max_dtau).or.(nlevel < min_lvl))).or.(lvl(ncurr) < nlevel))  ! do not remove term (lvl(ncurr) < nlevel). Needed for smooth cell refinement algorithm. 
 case('cavity', 'wind')
    rad=sqrt((x/ax)**2+(y/by)**2+(z/cz)**2)
    subdivision= (((nlevel < max_lvl).and.(((tau > max_dtau).and.(rad < R_subd_lim)).or.(nlevel < min_lvl))).or.(lvl(ncurr) < nlevel))
!!$ elseif (dust_geometry == 'wind') then 
!!$    rad=sqrt((x/ax)**2+(y/by)**2+(z/cz)**2)
!!$    subdivision= (((nlevel < max_lvl).and.(((tau > max_dtau).and.(rad < 1.2)).or.(nlevel < min_lvl))).or.(lvl(ncurr) < nlevel))   
 case default
...
write(42,*) ' dust geometry = ', dust_geometry
write(42,*) ' a =', ax, ' b =', by, 'c =', cz
!!$write(42,*) ' R_ax_ratio = ', R_ax_ratio
write(42,*) ' Ellisoid shell width = ', elrad_width
!!$write(42,*) ' Rad_ring =', rad_ring
!!$write(42,*) ' W ring = ', w_ring, 'H ring =', h_ring
write(42,*) ' GAS mass = ', tot_gas_mass, 'M(sun)'
write(42,*) ' DUST mass = ', tot_dust_mass, 'M(sun)'
write(42,*) ' modelsize =',  modelsize
write(42,*) ' tau_z =', tau_z
write(42,*) ' lambda_ref=', lambda_ref  
...
END PROGRAM create_adap_grid_magtar
